...
(
    ...
    localparam PAD28 = log2ceil(64'ha18 - 64'ha10); 
    localparam PAD29 = log2ceil(64'hb10 - 64'hb00); 
    localparam PAD30 = log2ceil(64'hb20 - 64'hb10); 
    localparam PAD31 = log2ceil(64'hc10 - 64'hc00); 
    // -------------------------------------------------------
    ...
    // ( 0x9a0 .. 0x9b0 )
    if ( {address[RG:PAD21],{PAD21{1'b0}}} == 12'h9a0  && read_transaction  ) begin
            src_channel = 32'b00000000000000000001000000000000;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 26;
    end

    // ( 0x9b0 .. 0x9c0 )
    if ( {address[RG:PAD22],{PAD22{1'b0}}} == 12'h9b0   ) begin
            src_channel = 32'b00000000000000000000100000000000;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 11;
    end

    // ( 0x9c0 .. 0x9d0 )
    if ( {address[RG:PAD23],{PAD23{1'b0}}} == 12'h9c0  && read_transaction  ) begin
            src_channel = 32'b00000000000000000000001000000000;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 10;
    end

    // ( 0x9d0 .. 0x9e0 )
    if ( {address[RG:PAD24],{PAD24{1'b0}}} == 12'h9d0   ) begin
            src_channel = 32'b00000000000000000000000010000000;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 13;
    end

    // ( 0x9e0 .. 0x9f0 )
    if ( {address[RG:PAD25],{PAD25{1'b0}}} == 12'h9e0  && read_transaction  ) begin
            src_channel = 32'b00000000000000000000000001000000;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 9;
    end

    // ( 0x9f0 .. 0xa00 )
    if ( {address[RG:PAD26],{PAD26{1'b0}}} == 12'h9f0   ) begin
    ...
    // ( 0xa10 .. 0xa18 )
    if ( {address[RG:PAD28],{PAD28{1'b0}}} == 12'ha10  && write_transaction  ) begin
            src_channel = 32'b00000000000000000000000000000001;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 1;
    end

    // ( 0xb00 .. 0xb10 )
    if ( {address[RG:PAD29],{PAD29{1'b0}}} == 12'hb00   ) begin
            src_channel = 32'b00100000000000000000000000000000;
            src_data[PKT_DEST_ID_H:PKT_DEST_ID_L] = 16;
    end

    // ( 0xb10 .. 0xb20 )
    if ( {address[RG:PAD30],{PAD30{1'b0}}} == 12'hb10   ) begin
